...
test-gomodules-integration:
	$(GOTEST) -v -tags="integration,gomodules" -timeout=10m ./pkg/ecosystems/go/modules/ -coverprofile cp.out

update-gradle-fixtures:
	UPDATE_FIXTURES=1 $(GOTEST) -v -tags="integration,gradle" -timeout=15m ./pkg/ecosystems/gradle/

.PHONY: install-req fmt test test-bazel-jvm-integration test-bazel-go-integration test-python-integration test-gradle-integration test-gomodules-integration update-gradle-fixtures lint tidy imports install-golangci-lint
