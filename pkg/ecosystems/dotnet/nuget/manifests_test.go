...
func TestReadProjectJSON(t *testing.T) {
  ...
  "frameworks": {"net45": {"dependencies": {"Nested": "2.0"}}}
		...
		},
		{
			...
			content:  `{"dependencies": {"Numeric": 5}}`,
			packages: []declaredPackage{{"Numeric", "5"}},
		},
		{
...
func TestReadProjectJSON_Rejects(t *testing.T) {
	tests := map[string]string{
		// project.json is a common enough file name that other ecosystems use
		// it — an Nx workspace most notably. Resolving one of those as a .NET
		// project would report nonsense rather than nothing.
		"an Nx project file":       `{"name": "web", "projectType": "application", "targets": {}}`,
		"an unrelated JSON object": `{"hello": "world"}`,
		"not JSON at all":          `<packages></packages>`,
		"empty":                    "",
	}

	for name, content := range tests {
		t.Run(name, func(t *testing.T) {
			path := writeTemp(t, projectJSONFile, content)
...
// A file that belongs to another ecosystem is told apart from one that is
// genuinely broken, so that discovering a few hundred Nx project.json files
// does not read as a few hundred failures.
func TestReadProjectJSON_OtherEcosystemsAreNotFailures(t *testing.T) {
	notOurs := writeTemp(t, projectJSONFile, `{"name": "web", "projectType": "application", "targets": {}}`)

	_, err := readProjectJSON(notOurs, projectJSONFile)
	require.Error(t, err)
	assert.ErrorIs(t, err, errNotDotnetManifest)

	broken := writeTemp(t, projectJSONFile, `{"dependencies": `)

	_, err = readProjectJSON(broken, projectJSONFile)
	require.Error(t, err)
	assert.NotErrorIs(t, err, errNotDotnetManifest, "unparseable JSON is a real failure")
