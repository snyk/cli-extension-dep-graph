...
| `sdk-injected-packages` | A `PublishAot` project, in the shape SDK 8.0.409 writes: the two compiler packs are `autoReferenced` and resolve to a `.props` file apiece, so they are left out of the graph while the package the project declared stays. The `autoReferenced` packages that *are* reported — `NETStandard.Library`, `Microsoft.NETFramework.ReferenceAssemblies` — are unit tests, since each needs its own target framework. |
