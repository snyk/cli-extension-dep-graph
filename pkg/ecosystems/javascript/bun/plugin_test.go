...
func TestPlugin_Simple_ProductionDepsOnly(t *testing.T) {
	...
	// Production deps present.
	assert.True(t, pkgIDs["debug@4.4.3"])
	assert.True(t, pkgIDs["ms@2.1.3"])

	// Dev deps excluded (not in seeds).
	assert.False(t, pkgIDs["@types/bun@1.3.11"])
}

func TestPlugin_Simple_IncludeDev(t *testing.T) {
	plugin := newPlugin(&fakeExecutor{outputFile: "testdata/simple/why_output.txt"})
	...
	assert.True(t, pkgIDs["@types/bun@1.3.11"], "dev dep should be included when --dev is set")
}
