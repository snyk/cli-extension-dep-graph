...
// packageJSON captures the fields we need from a package.json file.
...
// workspacesField handles both shapes Yarn accepts:
//   - ["packages/*"]                (legacy, classic + berry)
//   - {"packages": ["packages/*"]}  (extended, used for "nohoist" config)
...
func (w *workspacesField) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &obj); err != nil {
		return fmt.Errorf("workspaces field must be an array or {packages: []}: %w", err)
	}
