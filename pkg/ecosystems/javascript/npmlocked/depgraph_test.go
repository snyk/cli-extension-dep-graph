...
func TestSplitPkgID(t *testing.T) {
	tests := []struct {
		...
		// Workspace dir containing '@' (e.g. packages/@scope/pkg) — splitting
		// on the last '@' would mangle the name; "@file:" detection prevents it.
		{"@workspace/logger@file:packages/@scope/logger", "@workspace/logger", "file:packages/@scope/logger"},
		{"plain-name@file:apps/@org/web", "plain-name", "file:apps/@org/web"},
